...
def train_ngp(iterations):
    ...
    gui.render(iterations) # 500 to reach 0.55 progress
    return gui

def compute_densities(gui):
    for i in range(6):
        max_density = -1
        timestamp = -1

        gui.update_latent_from_image(f'v2-t{i}')
        gui.render_bev(dest_path=f'rendered_t{i}.png', color_t=i)
...
def train_and_compute_densities(iterations):
    gui = train_ngp(iterations)
    
    compute_densities(gui)

    return gui

def compute_probe_accuracy(n, gui):
    print("Num samples:", n)

    rand_t = np.random.choice(a=[1, 4], p=[0.5, 0.5])
    gui.render_bev(dest_path=f'rendered_t{rand_t}_pred.png', color_t=rand_t)
